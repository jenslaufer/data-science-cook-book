...
```{r}
df %>%
  count(HouseStyle, Condition1) %>%
  ggplot(aes(x=HouseStyle, y=Condition1)) +
    geom_tile(aes(fill=n))
```
...
df %>%
  count(HouseStyle, Condition1) %>%
  ggplot(aes(x=HouseStyle, y=n)) + 
    geom_bar(aes(fill = Condition1), stat ='identity', position = "dodge")
```
...
qualitative %>%
  ...
  count(HouseStyle, var, val) %>%
  ggplot(aes(x=HouseStyle, y=n)) + 
    geom_bar(aes(fill = val), stat ='identity', position = "dodge") +
    facet_wrap(~var, scales = 'free', ncol=4)
```